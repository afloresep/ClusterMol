...
Requires TMAP installed (python < 3.8)
"""
...
if __name__=="__main__":
    ...
    if args.dataframe is not None:
        ...
        if args.column is None:
            create_tmap(df['SMILES'], fingerprint=args.fingerprint, tmap_name=f'tmap_default')
        else:
            for cluster in df[args.column].unique():
                print('TMAP for ', cluster, args.column, len(df[df[args.column]==cluster]['SMILES']))
